# SnapTax Nutzungsbedingungen

**Zuletzt aktualisiert:** Juni 2026

## Betreiber & Kontakt

SnapTax wird betrieben von Gang Huang, Einzelunternehmer („wir", „uns", „unser").

Kontakt: snaptax.lightxforge@gmail.com  
Datenschutzanfragen: snaptax.lightxforge@gmail.com (Betreff: Privacy Request)  
Postanschrift: Hong Kong

Die Dienste werden Nutzern in den Vereinigten Staaten angeboten. Kundendaten werden in den Vereinigten Staaten verarbeitet und gespeichert, wie unten beschrieben.

Mit Nutzung von SnapTax („die App") stimmen Sie diesen Bedingungen zu. Bei Nichtzustimmung nutzen Sie die App nicht.

## 1. Vereinbarung & Leistungsbeschreibung

SnapTax hilft Selbstständigen und Vertragsarbeitern, Belege zu fotografieren, Ausgaben per KI zu kategorisieren und Tabellen für die Steuervorbereitung zu exportieren. Die App ist ein **Werkzeug**, keine Steuer-, Rechts- oder Buchführungsberatung. Wenden Sie sich an einen qualifizierten Steuerberater.

## 2. Berechtigung & zulässige Nutzung

Sie müssen in Ihrer Rechtsordnung einen verbindlichen Vertrag schließen können. Sie verpflichten sich, die App nicht zu missbrauchen, keine rechtswidrigen Inhalte hochzuladen, auf Daten anderer Nutzer zuzugreifen oder unsere Systeme zu stören.

## 3. Konten

- Anonyme Nutzung (Ghost-Konto) mit Gerätespeicher bis zur Anmeldung möglich.
- **Google-Anmeldung** ist die einzige Kontomethode. Google-Verknüpfung ermöglicht Cloud-Backup und Multi-Device-Zugriff.
- **Geräteverlust:** Ohne Anmeldung sind lokale Daten **nicht wiederherstellbar**. Wir warnen in der App vor Export und in den Einstellungen.

## 4. Abonnements & Zahlungen

Der Steuersaison-**Tax-Pack-Export** erfordert eine Einmalgebühr über **Paddle** (49 $ pro Saison, sofern in der App nicht anders angezeigt). Erstattungen folgen Paddle-Richtlinien und geltendem Recht. Bezahlte Saisons umfassen unbegrenzte Re-Exporte für diese Steuersaison. Siehe [Erstattungsrichtlinie](/refund) und [Preise](/pricing).

## 5. Geistiges Eigentum

SnapTax, Logo und Software gehören uns oder unseren Lizenzgebern. Sie behalten das Eigentum an Ihren Beleginhalten. Sie gewähren uns eine beschränkte Lizenz zur Verarbeitung Ihrer Belege ausschließlich zur Erbringung des Dienstes.

## 6. Haftungsausschluss & Steuerschätzungen

**Est. Tax Saved** und zugehörige Werte sind **nur Schätzungen**, keine Steuerberatung, Prüfungsverteidigung oder garantierte Ersparnisse. Kategorisierung kann falsch sein; Sie sind für die Überprüfung von Abzügen mit einem Fachmann verantwortlich.

DIE APP WIRD **„WIE BESEHEN"** OHNE GEWÄHRLEISTUNG BEREITGESTELLT. WIR HAFTEN NICHT FÜR INDIREKTE SCHÄDEN, STEUERSTRAFEN ODER VERLUSTE DURCH FALSCH KLASSIFIZIERTE BELEGE.

## 7. Haftungsbeschränkung

IM GESETZLICH ZULÄSSIGEN UMFANG IST UNSERE GESAMTHAFTUNG FÜR ANSPRÜCHE AUS DEM DIENST AUF DIE IN DEN **LETZTEN 12 MONATEN** GEZAHLTEN GEBÜHREN BEGRENZT.

## 8. Freistellung

Sie stellen uns von Ansprüchen frei, die aus Missbrauch der App oder Verstoß gegen diese Bedingungen entstehen, soweit gesetzlich nicht untersagt.

## 9. Kündigung & Sperrung

Wir können den Zugang bei Verstößen gegen diese Bedingungen oder zum Schutz des Dienstes sperren oder beenden. Sie können die App jederzeit nicht mehr nutzen und Daten über **Konto löschen** in den Einstellungen entfernen.

## 10. Streitbeilegung & anwendbares Recht

Diese Bedingungen unterliegen dem Recht des **Bundesstaates Delaware, USA**, unter Ausschluss kollisionsrechtlicher Regeln. EU-Verbraucher behalten zwingende lokale Rechte und DSGVO-Rechte. Streitigkeiten zuerst an **snaptax.lightxforge@gmail.com**.

## 11. Datenschutz

Ihre Nutzung unterliegt unserer [Datenschutzerklärung](/privacy). Mit dem Online-Fotografieren eines Belegs erkennen Sie diese Bedingungen und die Datenschutzerklärung einschließlich US-Verarbeitung an.

## 12. Kontakt

**snaptax.lightxforge@gmail.com**
