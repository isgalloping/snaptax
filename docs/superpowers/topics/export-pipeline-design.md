...
**Out of scope（永久）：** `03_Mileage_Log.csv`、TXF TD 2214 里程行、**QBO/OFX**（QIF shipped 2026-07-11）。
...
|--------|------------|---------------|
| `csv` · `txf` · `qif` | **Local IDB** — `buildLocalTaxPack` / `runLocalTaxExport` | `POST /api/export/filed` + IDB `markReceiptsFiledLocal` |
...
Generate (all visible formats)
  ...
  → csv/txf/qif: buildLocalTaxPack · cpa_*: runLocalCpaExport
...
- QBO / OFX（QuickBooks Online Web Connect；QIF shipped 2026-07-11）
