...
```json
{
  ...
  "orphanGhostIds": ["ghost_prev_1", "ghost_prev_2"]
}
```
...
4. 合并 orphan ghosts：来源为本次 rebind 的旧 ghost、服务端历史 `ghost_id`、以及客户端 `orphanGhostIds`（最多 20 个；由 `snap1099_known_ghost_ids` 推导）。
...
Google 登录后 best-effort 合并“轮换过但未绑定”的 Ghost 数据（receipts + Event Store）。客户端在 `ensureGhostSession()` 后读取 `snap1099_known_ghost_ids`，排除当前 Ghost 后提交；接口也会补充服务端历史 ghosts。
...
| 字段 | 约束 |
|------|------|
| `orphanGhostIds` | 可省略；最多 20 个非空字符串 |
