...
交互：`useTaxExportGate` 统一主屏 Export 与 Settings 入口 → `ExportEngineSheet` 3–4 步 Bottom Sheet（税年 → 可选分类复核 → 格式 → 生成/分享）。Gate 前 **`prepareExportLocal`**（flush + IDB）；全部格式 Generate 前均 **`prepareExportLocal`**（见 §3.8）。
...
| `qif` | QuickBooks QIF | 第 5 按钮（M4b · 2026-07-11） |
| `qbo` | QuickBooks Online Web Connect | 第 6 按钮（M4c · 2026-07-12） |
...
| [`docs/tech/08-export.md`](../../tech/08-export.md) | **API 契约** — local-first `/api/export/filed`、`POST /api/export/tax-pack` fallback、格式表、错误码 |
...
Export tap (Home / Settings)
  ...
  → prepareExportLocal (flush upload → flush delete → load IDB)
...
| Eligibility | `lib/export/auditEligibleRows.ts` — `exportEligibleRows = taxDeductible !== "No" && exportAmount > 0` |
| CSV | `lib/tax/exportCsv.ts` — TurboTax 8 列、无 BOM |
| TXF | `lib/export/buildTxf.ts` |
| QIF | `lib/export/buildQifExport.ts` — `!Type:Cash`、负 `T`、Schedule C `L` 行、memo `SNPTX{receiptId}` |
| QBO | `lib/export/buildQboExport.ts` — OFX SGML + `INTU.BID`、DEBIT `STMTTRN`、稳定 `FITID`、memo `Line N - Category` |
...
- `exportEligibleRows` 是 TXF / QIF / QBO / CPA audit 的共享 predicate：`taxDeductible !== "No" && exportAmount > 0`。
- Personal / 零抵扣 **排除** 于 PDF appendix、ZIP 费用图、Detail CSV、Index；TurboTax CSV 仍含 Personal（既有行为）。
- TXF builder 会跳过非 eligible rows；若该税年只有 Personal rows，TXF 可能只含 header。QIF/QBO 在本地与 server route 都会对零 eligible rows 返回 `NO_RECEIPTS`。

**Out of scope（永久）：** `03_Mileage_Log.csv`、TXF TD 2214 里程行、**通用 OFX**（QBO shipped 2026-07-12；QIF shipped 2026-07-11）。
...
|--------|------------|---------------|
| `csv` · `txf` · `qif` · `qbo` | **Local IDB** — `buildLocalTaxPack` / `runLocalTaxExport` | `POST /api/export/filed` + IDB `markReceiptsFiledLocal` |
...
Generate (all visible formats)
  ...
  → if csv/txf/qif/qbo:
       runLocalTaxExport
         → buildLocalTaxPack (initial file/NO_RECEIPTS)
         → POST /api/export/filed { taxYear }
         → markReceiptsFiledLocal(server receiptIds)
         → txf/qif/qbo rebuild from buildLocalExpenseExportRows
             (txf/qbo use filed.taxSeasonDate as export timestamp)
  → if cpa_pdf/cpa_pack:
       runLocalCpaExport
         → build browser PDF/ZIP
         → POST /api/export/filed { taxYear }
         → markReceiptsFiledLocal(server receiptIds)
...
- `buildLocalExpenseExportRows` 使用本地 receipts：`status=done`、排除 1099 income categories、`filterReceiptsByTaxYear`、`dataRegion: "us"`、`finalizeExportRows`。
- `/api/export/filed` 的 filed 范围是该税年全部 server `status=done` receipts（user + bound ghost），可能大于实际导出文件中的 eligible rows。
- `runLocalTaxExport` 的 `meta.receiptCount` = `/filed` 返回的 `filedCount`，不是 QIF/QBO/TXF 文件行数。

**Modules:** `lib/export/buildLocalExpenseRows.ts` · `lib/export/buildLocalTaxPack.ts` · `lib/client/runLocalTaxExport.ts` · `lib/client/runLocalCpaExport.ts` · `lib/export/buildBrowserScheduleCMirrorPdf.ts` · `lib/export/buildLocalCpaPackZip.ts` · `lib/client/exportPrepareFlow.ts` · `app/api/export/filed/route.ts`
...
- 通用 OFX（非 QBO Web Connect；QBO shipped 2026-07-12）
...
| M4b QuickBooks QIF（`buildQifExport` · PR #152） | Done · 2026-07-11 |
| M4c QuickBooks Online QBO（`buildQboExport`） | Done · 2026-07-12 |
