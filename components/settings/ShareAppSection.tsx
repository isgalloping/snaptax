...
export function ShareAppSection() {
  ...
  const handleWhatsApp = () => {
    ...
    openExternalShare(buildWhatsAppShareUrl(payload.combinedText));
  };
  ...
  const tileClass =
    "flex min-h-20 flex-col items-center justify-center gap-2 rounded-xl border-2 bg-zinc-800 p-3 text-xs font-bold transition-transform active:scale-95";
  ...
  return (
    <section className="mb-8">
      ...
      <p className="mb-4 text-sm text-zinc-400">{copy.hint}</p>
      <div className="grid grid-cols-3 gap-3">
        <button
          type="button"
          onClick={handleWhatsApp}
          className={`${tileClass} border-[#25D366] text-[#25D366]`}
        >
          <WhatsAppIcon />
          {copy.whatsapp}
        </button>
        <button
          ...
          className={`${tileClass} border-[#1877F2] text-[#1877F2]`}
        >
          <FacebookIcon />
          {copy.facebook}
        </button>
        <button
          ...
          className={`${tileClass} border-zinc-600 text-white`}
        >
          <MoreShareIcon />
          {copy.more}
        </button>
