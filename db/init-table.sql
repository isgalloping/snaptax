...
CREATE TABLE snaptax_users (
  ...
  founder_tier          VARCHAR(32),
  founder_status        VARCHAR(16),
  founder_locked_at     TIMESTAMPTZ(3),
...
CREATE INDEX snaptax_users_founder_number_idx
...
COMMENT ON COLUMN snaptax_users.founder_number IS 'Founder 席位编号 1–50；首次 qualifying 购买原子分配；null 表示非 Founder';
COMMENT ON COLUMN snaptax_users.founder_tier IS 'Founder SKU 档位；应用层枚举：FOUNDER_LEVEL_SUPER, EARLY, FOUNDER, DEFAULT';
COMMENT ON COLUMN snaptax_users.founder_status IS 'Founder 订阅状态；应用层枚举：active, lapsed；null 表示从未成为 Founder';
COMMENT ON COLUMN snaptax_users.founder_locked_at IS '首次 Founder 购买锁定 tier 的时间（TIMESTAMPTZ UTC）';
