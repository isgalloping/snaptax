...
| 2 | **Security by Default** | TLS（Vercel）；云端 AES 声明；OPFS AES-GCM；OAuth；Paddle 签名校验；HMAC Ghost | 无统一 Security Baseline 文档；最小权限/RBAC 未成文 | `security-baseline.md` + 代码对照表 | 子处理方 + 传输/静态加密 + 认证方式可审计；Webhook 测试覆盖 | P3 | M2 |
...
| 10 | **Data Retention** | 代码：18mo 小票、90d OPFS、rate-limit GC | **无法律层**统一说明；服务端 PG/Blob 周期未披露 | `docs/legal/data-retention.md` + Privacy 链接 | 文档周期 = 代码常量；Delete Account 说明即时删 | P2 | M2 |
