...
export function ReceiptCard({ receipt, onResnap }: ReceiptCardProps) {
  if (receipt.status === "processing") {
    return (
      <div className="flex items-center justify-between rounded-xl border-l-4 border-yellow-500 bg-zinc-800 p-4">
        <div>
          <p className="text-lg font-extrabold text-yellow-500">Processing...</p>
          <p className="mt-0.5 text-xs text-zinc-400">
            Just now · {receipt.merchant ?? "Scanning"}
          </p>
        </div>
  ...
  return (
    <div className="flex items-center justify-between rounded-xl border-l-4 border-zinc-600 bg-zinc-800 p-4">
      <div>
        <p className="text-lg font-extrabold text-white">
          {formatCurrency(receipt.amount ?? 0)}
        </p>
        <p className="mt-0.5 text-xs text-zinc-400">
          {formatReceiptTime(receipt.timestamp)} · {receipt.merchant}
        </p>
      </div>
