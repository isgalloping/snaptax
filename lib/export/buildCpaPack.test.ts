...
describe("buildCpaPackZip", () => {
  it("uses PRD audit trail paths for income and expense images", async () => {
    ...
    assert.ok(
      zipContains(result.buffer, expenseRow.receiptArchivePath),
    );
    assert.equal(zipContains(result.buffer, "00_READ_ME_Summary.pdf"), false);
    assert.equal(zipContains(result.buffer, "Expenses-Detail.csv"), false);
    assert.equal(zipContains(result.buffer, "02_Expenses_Receipts_Book"), false);
